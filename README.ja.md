...
| 区分 | 内容 |
|---|---|
| **限界 1** | 評価セットが20問規模のため、実運用環境の多様なクエリパターンを完全には網羅できていない |
| **限界 2** | 定量指標（RAGAS）中心の評価であり、ユーザーが体感する定性的な満足度の検証が不十分 |
| **限界 3** | HtmlRAGなど既存手法との同一条件での直接比較が未実施 |
| **今後 1** | 実サービス公開後、ユーザーログに基づくHuman Eval · Context Precision/Recall の定期測定 |
| **今後 2** | AIエージェント（検索クローラー）検知時に精製済み**Markdownを動的配信**するGEOインフラの構築 |
| **今後 3** | データセット拡張による汎化性能の検証、および韓国語Web環境における検索アルゴリズムの限界分析 |
